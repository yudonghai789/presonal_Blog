---
layout: default
title: 余东海的 个人博客
---

# 欢迎来到我的 5G NR 技术博客

作为一名 NR 软件工程师，我在这里记录关于通信算法、信号处理和代码优化的点滴。

## 🚀 最新文章

### [2026-03-06] 傅里叶变换 (FFT) 与加窗实战
这篇文章详细介绍了 FFT 的物理意义，并通过 Python 演示了加窗对 16-QAM 星座图的影响。

* [点击这里查看完整 Notebook 内容](./fft/傅里叶变换基础知识总结.ipynb)
* [在线交互预览 (nbviewer)](https://nbviewer.org/urls/yudonghai789.github.io/presonal_Blog/new.ipynb)  
* [![Open In Colab 可运行python片段](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/yudonghai789/presonal_Blog/blob/master/fft/傅里叶变换基础知识总结.ipynb)


## 🚀 演示文稿
* [在线交互预览 (nbviewer):ipynb](https://nbviewer.org/github/yudonghai789.github.io/presonal_Blog/new.ipynb)
[![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/yudonghai789/presonal_Blog/blob/main/new.ipynb)

---
© 2026 余东海 | 基于 GitHub Pages 构建
